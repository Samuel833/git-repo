...
const Repos = () => {
    const { data: headerData, isLoading: headerLoader } = useFetchUrl(
        "https://api.github.com/users/Samuel833",
        "headerData"
    );
    const { data: reposData, isLoading: reposLoader } = useFetchUrl(
        "https://api.github.com/users/Samuel833/repos",
        "reposData"
    );
    ...
    return (
        <section className=' bg-white text-zinc-700 dark:bg-zinc-700 font-sans dark:text-neutral-200  after:content-[""] after:absolute after:w-full after:h-[50px] after:top-[29.25rem]  md:after:top-[6.3rem] after:left-0 border-grey-100 dark:after:border-neutral-400 px-8'>
